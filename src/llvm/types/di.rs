...
impl LLVMTypeWrapper for DICompositeType<'_> {
    ...
    fn from_ptr(value: NonNull<Self::Target>) -> Result<Self, LLVMTypeError> {
        let metadata = unsafe { LLVMValueAsMetadata(value.as_ptr()) };
        // PANICS: value is NonNull. Hence, the result of LLVMValueAsMetadata
        // should not be null, unless LLVM is severly broken.
...
impl DICompositeType<'_> {
    ...
    pub fn elements(&self) -> impl Iterator<Item = Metadata> {
        ...
        (0..operands).map(move |i| {
            ...
            // PANICS: We are sure that the pointer type is correct. There is
            // no need to leak the error.
            Metadata::from_value(operand).unwrap()
        })
    }
...
impl<'ctx> LLVMTypeWrapper for DISubprogram<'_> {
    type Target = LLVMValue;
