                    ...
                    Some(DW_TAG_structure_type) => {
                        ...
                        if num_elements > 0 {
                            ...
                            if get_tag(element) == Some(DW_TAG_variant_part) {
                                ...
                                let mut len = 0;
                                let line = LLVMDITypeGetLine(LLVMValueAsMetadata(value)); // always returns 0
                                let scope = LLVMDIVariableGetScope(metadata);
                                let file = LLVMDIScopeGetFile(scope);
                                let mut len = 0;
                                let filename = to_string(LLVMDIFileGetFilename(file, &mut len)); // still getting <undefined>
...
unsafe fn is_globalvariable(v: LLVMValueRef) -> bool {
    !LLVMIsAGlobalVariable(v).is_null()
}

unsafe fn is_function(v: LLVMValueRef) -> bool {
