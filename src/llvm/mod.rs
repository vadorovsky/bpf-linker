...
pub enum LlvmVersionDetectionError {
    #[error("failed to retrieve LLVM version from bitode: {0}")]
...
pub fn bitcode_llvm_version(buffer: &[u8]) -> Result<(u32, String), LlvmVersionDetectionError> {
...
fn parse_llvm_version_from_producer(
    ...
    let remainder = remainder.trim_start_matches(|c: char| c == ' ');
